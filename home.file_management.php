...
echo "<div style=\"width:96%;float:left;border-bottom:1px solid #CCC;padding:10px 2% 10px 2%;font-weight:bold;\">\n";
    ...
    echo "<div style=\"width:20%;float:left;\">\n";
    echo "<span style=\"cursor:pointer;\"
        onclick=\"$('#frame_to_fill').load('ajax_action.php?action=session_7');\">\n";
    echo "<div style=\"float:left;padding-right:4px;margin-top:-2px;\">\n";
    echo "<img src=\"images/icons/document--plus.png\" />\n";
    echo "</div>\n";
    echo "File";
    echo "</span>\n";
    echo "</div>\n";
...
$q = "SELECT * FROM file ORDER BY id DESC;";
$res = mysql_query($q);
...
foreach($item_id as $key => $val){
    if($item_type[$key]=="write_up") print_a_write_up_item($val);
    elseif($item_type[$key]=="event") print_an_event_item($val);
    elseif($item_type[$key]=="file") print_a_file_item($val);
}
